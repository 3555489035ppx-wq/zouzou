...
describe('journey image system', () => {
  ...
  it('rejects low-resolution, blurry, text, QR, screenshot, collage, black-border, decorative-border, signage and portrait candidates', () => { const result = inspectCandidate(candidate({ width: 800, height: 600, sharpnessScore: 20, hasOverlayText: true, hasQrCode: true, isScreenshot: true, isCollage: true, hasBlackBorder: true, hasDecorativeBorder: true, isSignage: true, aspectRatio: .76 }), journey); expect(result.reasons).toEqual(expect.arrayContaining(['too_small', 'blur', 'overlay_text', 'qr_code', 'screenshot', 'collage', 'black_border', 'decorative_border', 'signage', 'portrait'])) })
  ...
  it('does not mistake a scenic place containing a food character for a dining route', () => {
    const route: JourneyImageInput = { id: 'guangzhou-shamian', title: '广州沙面岛慢走', category: '旅行', city: '广州', places: ['沙面岛'] }
    const selected = selectJourneyCover(route, [{
      src: '/assets/journey-images/wikimedia/guangzhou-shamian-clean.jpg',
      alt: '广州沙面岛建筑与街景空景，不含人物',
      landmark: '沙面岛',
      kind: 'landmark',
      routeEligible: true,
      sourceUrl: 'https://commons.wikimedia.org/wiki/File:Shamian_Island.jpg',
      credit: 'Wikimedia Commons',
      license: '以来源页标注为准',
      licenseUrl: null,
      downloadUrl: 'https://commons.wikimedia.org/wiki/Special:FilePath/Shamian_Island.jpg',
    }])
    expect(selected.image?.cachedUrl).toBe('/assets/journey-images/wikimedia/guangzhou-shamian-clean.jpg')
  })
  ...
  it('blocks known people-heavy and watermarked static covers', () => {
    ...
    expect(isUserFacingCover('/assets/locations/harbin-cover-02.jpg')).toBe(false)
  })
  it('blocks manually reviewed decorative and unsuitable acquired covers', () => {
    expect(isUserFacingCover('/assets/journey-images/baidu/baidu-800abf6e-8.png')).toBe(false)
    expect(isUserFacingCover('/assets/journey-images/baidu/baidu-b9531692-22.png')).toBe(false)
    expect(isUserFacingCover('/assets/journey-images/baidu/baidu-988b0a1f-3.png')).toBe(false)
    expect(isUserFacingCover('/assets/journey-images/baidu/baidu-991403ef-5.jpg')).toBe(false)
    expect(isUserFacingCover('/assets/journey-images/baidu/baidu-7c86482c-14.jpg')).toBe(false)
  })
