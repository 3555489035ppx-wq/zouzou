...
export const FootprintPage = () => {
  ...
  return <AppShell showTabBar><ZouNavigationBar title="我的足迹" right={<button className="text-button" onClick={() => navigate('/journey/tools')}>行程工具</button>} /><main className="journey-form-page page-content">
    ...
    <section className="footprint-map" aria-label="我的足迹城市概览"><div className="footprint-map__grid" />{Object.entries(cityCounts).map(([name, count], index) => <button key={name} style={{ left: `${18 + (index * 29) % 68}%`, top: `${26 + (index * 23) % 48}%` }} onClick={() => setCity(name)}><span>{count}</span><strong>{name}</strong></button>)}{!Object.keys(cityCounts).length ? <p>标记第一个地点，地图就会亮起来</p> : null}</section>
