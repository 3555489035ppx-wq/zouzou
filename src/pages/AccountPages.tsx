...
export { StartupExperience as SplashPage } from '../components/StartupExperience'
...
export const LoginPage = () => {
  ...
  const continueAfterLogin = () => { if (onboardingCompleted) navigate(nextPath ?? '/home'); else navigate(nextPath ? `/onboarding?next=${encodeURIComponent(nextPath)}` : '/onboarding') }
  const continueWithProvider = async (provider: 'wechat' | 'apple') => {
    setError('')
    try { await localAuthAdapter.signInWithProvider(provider); continueAfterLogin() } catch(cause) { setError(cause instanceof Error ? cause.message : '登录未完成') }
  }
