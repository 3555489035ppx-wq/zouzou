...
export const TripsPage = () => {
  ...
  useEffect(() => {
    ...
    const segmentSeconds = places.length <= 3 ? 3.2 : places.length <= 5 ? 4.2 : 3.8
    const delta = (1 / Math.max(1, places.length - 1)) * (80 / (segmentSeconds * 1000))
  ...
  const togglePlayback = () => {
    const nextIndex = Math.min(places.length - 1, playback.activeIndex + 1)
    if (playback.state !== 'moving' && nextIndex > playback.activeIndex) setFocus([playback.activeIndex, nextIndex])
