...
import { Check, ChevronDown, MapPin, MoreHorizontal, Search, Send, Share2 } from 'lucide-react'
import { useNavigate, useParams } from 'react-router-dom'
import { AppShell } from '../components/AppShell'
import { RealRouteMap } from '../components/RealRouteMap'
import {
  DetailActionBar, ZouAvatar, ZouBottomSheet, ZouButton, ZouCommunityCard, ZouMotionBot,
...
const CommunityHeader = ({ active, onChange }: { active: string; onChange: (value: string) => void }) => {
  ...
  return <header className="community-header"><div className="community-header__top"><button aria-pressed={active === '关注'} onClick={() => onChange('关注')}>关注</button><button aria-pressed={active === '发现'} onClick={() => onChange('发现')}>发现</button><button onClick={() => setCity(communityCity === '上海' ? '杭州' : '上海')}>{communityCity}<ChevronDown /></button><button className="icon-button" aria-label="搜索社区" onClick={() => navigate('/community/search')}><Search /></button></div><div className="community-categories">{['推荐', '旅行', '周末', '约会', '聚餐'].map((item) => <button key={item} aria-pressed={active === item} onClick={() => onChange(item)}>{item}</button>)}</div></header>
}
...
export const CommunityPage = () => {
  ...
  return <AppShell showTabBar><div className="community-page"><CommunityHeader active={active} onChange={setActive} />{posts.length ? <CommunityMasonry posts={posts} onOpen={(post) => navigate(`/community/${post.id}/replay`)} /> : <div className="community-empty"><ZouMotionBot state="idle" size="sm" /><h2>关注页还没有内容</h2><p>关注一位作者后，他完成的行程会出现在这里。</p><ZouButton onClick={() => setActive('发现')}>去发现</ZouButton></div>}</div></AppShell>
}
...
const CommunityDetail = ({ post }: { post: CommunityPost }) => {
  ...
  const useTrip = async (mode: 'keep' | 'optimize') => {
    ...
    setUseOpen(false); setToast(mode === 'keep' ? '已保留原路线骨架' : '已按偏好局部优化')
  }
  return <div className="community-detail"><header className="detail-author"><ZouAvatar src={post.avatar} name={post.author} /><div><strong>{post.author}</strong><small>{post.city} · ✓ 已完成</small></div><button aria-pressed={followed} onClick={() => toggleFollow(post.author)}>{followed ? '已关注' : '关注'}</button><button className="icon-button" aria-label="分享"><Share2 /></button></header><ZouPhotoCarousel images={post.images} title={post.title} /><article className="detail-copy"><h1>{post.title}</h1><p>{post.summary}</p><dl><div><dt>时间</dt><dd>3 天 2 晚</dd></div><div><dt>花费</dt><dd>¥1860</dd></div><div><dt>步行</dt><dd>6.8 km</dd></div></dl><section className="community-detail__itinerary" aria-labelledby="community-itinerary-title"><header><h2 id="community-itinerary-title">具体行程</h2><span>{post.city} · DAY 1</span></header><ol>{itinerary.map((place, index) => <li key={place.id}><span>{index + 1}</span><div><strong>{place.name}</strong><small>{place.time} · {place.type} · {place.stay}</small><p>{place.transport}</p></div><b>¥{place.budget}</b></li>)}</ol></section><h2>这次最推荐</h2><p>武康路放在上午，下午转入室内展览。移动顺序没有折返，也给朋友留了自由逛店的时间。</p><h2>需要注意</h2><p>周末热门街区人多，咖啡店不建议排队超过 20 分钟。路线中准备了三个附近替代点。</p></article><DetailActionBar post={post} onUse={() => setUseOpen(true)} onComments={() => setCommentsOpen(true)} /><ZouBottomSheet open={commentsOpen} onClose={() => setCommentsOpen(false)} title="评论"><div className="comment-list"><article><ZouAvatar src="/assets/coffee.jpg" name="林晓" size="sm" /><p><strong>林晓</strong>下午去美术馆这个安排很聪明。</p><button>回复</button></article><article><ZouAvatar src="/assets/weekend.jpg" name="周周" size="sm" /><p><strong>周周</strong>收藏了，下周按这个路线走。</p><button>回复</button></article></div><label className="comment-input"><span className="sr-only">写评论</span><input placeholder="写下评论" /><button aria-label="发送评论"><Send /></button></label></ZouBottomSheet><ZouBottomSheet open={useOpen} onClose={() => setUseOpen(false)} title="使用这个行程"><button className="use-trip-option" onClick={() => useTrip('keep')}><strong>尽量保持原路线</strong><span>保留地点顺序和节奏，只重新检查日期。</span></button><button className="use-trip-option" onClick={() => useTrip('optimize')}><strong>按我的偏好重新优化</strong><span>保留路线骨架，只替换真正不合适的节点。</span></button><ZouButton variant="plain" onClick={() => { setUseOpen(false); navigate('/travel/new') }}>先看看输入</ZouButton></ZouBottomSheet>{toast ? <ZouToast message={toast} onClose={() => setToast('')} /> : null}</div>
}
...
export const CommunityDetailPage = () => {
  const { id } = useParams()
  const navigate = useNavigate()
  useEffect(() => { navigate(`/community/${id}/replay`, { replace: true }) }, [id, navigate])
  return null
}

export const PublishPage = () => {
  const navigate = useNavigate()
  const [cover, setCover] = useState('/assets/route-overview.svg')
  const [title, setTitle] = useState('上海三天两晚：把雨留给展览')
  const [published, setPublished] = useState(false)
  const covers = ['/assets/route-overview.svg', '/assets/wukang-road.jpg', '/assets/museum.jpg']
