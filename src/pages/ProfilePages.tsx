...
export const PeopleListPage = ({ title }: { title: '关注' | '粉丝' }) => <AppShell><ZouNavigationBar title={title} /><div className="page-content people-list">{['林晓', '周周', '安安', '阿柚', '野生小海'].map((name, index) => <article key={name}><ZouAvatar src={communityPosts[index].avatar} name={name} /><div><strong>{name}</strong><small>{index % 2 ? '上海 · 12 条行程' : '杭州 · 8 条行程'}</small></div><button>{title === '关注' ? '已关注' : '关注'}</button></article>)}</div></AppShell>
