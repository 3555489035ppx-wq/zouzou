...
test('社区卡片先回放再进入详情并可使用行程', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '使用这个行程' })).toBeVisible({ timeout: 14_000 })
  ...
  await expect(page.getByRole('status')).toContainText('已保留原路线骨架')
  ...
  await expect(page.getByText('下午去美术馆这个安排很聪明。')).toBeVisible()
})
...
test('方案地点可局部替换并明确第三方边界', async ({ page }) => {
  ...
  await expect(page.getByRole('status')).toContainText('尚未接入真实服务')
})
