...
test('发现卡片直接进入详情、可互动并可选回放', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /行程进度/ }).click()
  await expect(page).toHaveURL(/\/replay/)
  await expect(page.locator('.route-overview')).toBeVisible({ timeout: 12_000 })
...
test('开发中心可直达异常和完成状态', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '定位提示' })).toBeVisible()
  await expect(page.getByText(/不会在页面内模拟导航路线|仍可手动选择城市/)).toBeVisible()
})
...
test('今日进度、当前地点外部地图打开与定位提示', async ({ page }) => {
  await page.goto('/trips')
  const progress = page.locator('.trip-progress-card')
  await expect(progress).toBeVisible({ timeout: 12_000 })
  await expect(progress).toContainText('今日进度')
  await expect(page.locator('.route-overview')).toHaveCount(0)
  await expect(page.getByText('地点顺序概览', { exact: true })).toHaveCount(0)
  await progress.getByRole('button', { name: /打开.*地图/ }).click()
  ...
  await expect(page.getByText('已到达当前站', { exact: true })).toBeVisible({ timeout: 5_000 })
...
test('切换到南京后，行程主页不会继续显示上海地点', async ({ page }) => {
  ...
  await expect(page.locator('.trip-itinerary')).toContainText('颐和路')
})

test('行程页只保留今日进度和今日行程', async ({ page }) => {
  await page.goto('/trips')
  await expect(page.locator('.trip-progress-card')).toBeVisible({ timeout: 12_000 })
  await expect(page.locator('.trip-day-summary')).toBeVisible()
  await expect(page.locator('.trip-itinerary')).toBeVisible()
  await expect(page.locator('.route-overview')).toHaveCount(0)
  await expect(page.getByText('地点顺序概览', { exact: true })).toHaveCount(0)
  await expect(page.getByRole('button', { name: /打开.*地图/ }).first()).toBeVisible()
})
